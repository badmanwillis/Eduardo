...
#include <sensor_msgs/JointState.h>
void jointCb(const sensor_msgs::JointState& cmd_msg)
{
    //str_msg.data = hello;
    //chatter.publish( &str_msg ); 
    //myled = !myled;
    
    // Code here to take the joint_states and put them into a means that can be used by the code to move the arm.
    // Splitting the input data, putting data into variables eg:
    
    // hand_goal = joint_states[element of array for hand joint state];
    
}
ros::Subscriber<sensor_msgs::JointState> sub2("joint_states", &jointCb);
...
int main() {
    ...
    nh.subscribe(sub);
    nh.subscribe(sub1);
    nh.subscribe(sub2); //joint_states
    nh.advertise(flex);
    ...
    while(1){
        ...
        while (execute == 1) {
            ...
            if(prismatic_pos == prismatic_goal && revolute_pos == revolute_goal && elbow <= elbow_plus && elbow >= elbow_minus && wrist <= wrist_plus && wrist >= wrist_minus && hand <= hand_plus && hand >= hand_minus){
                // goal reached
                green = 0;
                execute = 0;
